...
-- Liest einen JSON-String ab der Position des oeffnenden Anfuehrungszeichens.
-- Damit vermeiden wir absichtlich komplizierte Lua-Patterns fuer escaped Zeichen.
...
local function response_output_text(raw)
  -- Die Responses API liefert output_text auch als oberstes Feld.
  local key_start, key_end = raw:find('"output_text"', 1, true)
  if key_start then
    local colon = raw:find(":", key_end + 1, true)
    local quote = colon and raw:find('"', colon + 1, true)
    local text = quote and read_json_string(raw, quote)
    if text and text ~= "" then return text end
  end

  -- Fallback: erstes content-Objekt vom Typ output_text.
  local search_from = 1
  while true do
    local type_start, type_end = raw:find('"type"', search_from, true)
    if not type_start then break end
    local next_part = raw:sub(type_end + 1, math.min(#raw, type_end + 120))
    if next_part:find('"output_text"', 1, true) then
      local text_key_start, text_key_end = raw:find('"text"', type_end + 1, true)
      if text_key_start then
        local colon = raw:find(":", text_key_end + 1, true)
        local quote = colon and raw:find('"', colon + 1, true)
        local text = quote and read_json_string(raw, quote)
        if text and text ~= "" then return text end
      end
    end
    search_from = type_end + 1
  end
  return nil
end
    ...
    if take and reaper.TakeIsMIDI(take) then
      ...
      for n = 0, (note_count or 0) - 1 do
        ...
        if ok and not muted then
          ...
          notes[#notes+1] = {start_qn=start_qn, duration_qn=end_qn-start_qn, pitch=pitch, velocity=velocity, channel=channel}
        end
      end
      items[#items+1] = {
        item=item, take=take, track=track, guid=get_guid(item),
...
local function context_text(items)
  ...
  for i, item in ipairs(items) do
    lines[#lines+1] = string.format("ITEM %d id=%s track=%s take=%s", i, item.guid, item.track_name, item.take_name)
    for _, note in ipairs(item.notes) do
      lines[#lines+1] = string.format("N %.5f %.5f %d %d %d", note.start_qn, note.duration_qn, note.pitch, note.velocity, note.channel)
    end
  end
...
local function ask_request(count)
  local ok, text = reaper.GetUserInputs(SCRIPT_NAME .. " " .. VERSION, 1, string.format("%d MIDI-Item(s) erkannt. Freier Kompositionsauftrag:,extrawidth=420", count), "")
...
local function run_openai(prompt, key)
  ...
  local request_path, response_path, status_path = base..".json", base..".out", base..".code"
  ...
  if status ~= "200" or not raw then
    return nil, "KI-Aufruf fehlgeschlagen (HTTP " .. tostring(status) .. ")."
  end
  local text = response_output_text(raw)
  if not text then return nil, "Keine Textantwort der KI gefunden." end
  return text
end
...
if not key then reaper.ShowMessageBox(key_error, SCRIPT_NAME, 0); return end
