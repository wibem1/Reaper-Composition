...
local open,input,busy=true,"",false; local restarting=false; local update_status=""; local history={}; local chat_start=1; local info_visible=false; local history_mode=false; local current_project=reaper.EnumProjects(-1,""); local font=nil
...
local function compact_context(items,tracks) tracks=tracks or selected_tracks(); local l={string.format("Tempo %.2f BPM; selected MIDI items=%d; selected tracks=%d",reaper.Master_GetTempo(),#items,#tracks),time_selection_context()}; for i,t in ipairs(tracks) do l[#l+1]=string.format("TRACK %d id=%s name=%s index=%d",i,t.guid,t.name,t.index) end; for i,it in ipairs(items) do l[#l+1]=string.format("ITEM %d id=%s trackId=%s track=%s take=%s rangeQN=%.3f..%.3f",i,it.guid,it.track_guid,it.track_name,it.take_name,it.start_qn,it.end_qn) end; return table.concat(l,"\n") end
local function music_context(items,tracks) local l={compact_context(items,tracks)}; for _,it in ipairs(items) do for _,n in ipairs(it.notes) do l[#l+1]=string.format("N %s %.5f %.5f %d %d %d",it.guid,n.start_qn,n.duration_qn,n.pitch,n.velocity,n.channel) end end; return table.concat(l,"\n") end
...
local function existing_program(track)
 for i=0,reaper.CountTrackMediaItems(track)-1 do local item=reaper.GetTrackMediaItem(track,i); local take=item and reaper.GetActiveTake(item); if take and reaper.TakeIsMIDI(take) then local _,_,_,cc=reaper.MIDI_CountEvts(take); for n=0,(cc or 0)-1 do local ok,_,muted,_,chanmsg,_,msg2=reaper.MIDI_GetCC(take,n); if ok and not muted and chanmsg==0xC0 then return msg2 end end end end; return nil
end
local function program_for_name(name,proposed)
 local n=(name or ""):lower()
 local map={{"violin",40},{"violine",40},{"geige",40},{"viola",41},{"bratsche",41},{"cello",42},{"violoncello",42},{"kontrabass",43},{"double bass",43},{"gitarre",24},{"guitar",24},{"harfe",46},{"harp",46},{"flöte",73},{"floete",73},{"flute",73},{"oboe",68},{"klarinette",71},{"clarinet",71},{"fagott",70},{"bassoon",70},{"trompete",56},{"trumpet",56},{"horn",60},{"posaune",57},{"trombone",57},{"sax",65},{"klavier",0},{"piano",0},{"orgel",19},{"organ",19}}
 for _,p in ipairs(map) do if n:find(p[1],1,true) then return p[2] end end
 local v=tonumber(proposed); if v and v>=0 and v<=127 then return math.floor(v) end
 return 0
end
local function create_midi(track,name,notes,program) local lo,hi=math.huge,-math.huge; for _,n in ipairs(notes) do lo=math.min(lo,n.start_qn); hi=math.max(hi,n.start_qn+n.duration_qn) end; if hi<=lo then return nil end; local item=reaper.CreateNewMIDIItemInProj(track,reaper.TimeMap2_QNToTime(0,lo),reaper.TimeMap2_QNToTime(0,hi),false); local take=item and reaper.GetActiveTake(item); if not take then return nil end; reaper.GetSetMediaItemTakeInfo_String(take,"P_NAME",name,true); local ch=math.max(0,math.min(15,(notes[1].channel or 0))); local ppq=reaper.MIDI_GetPPQPosFromProjTime(take,reaper.TimeMap2_QNToTime(0,lo)); reaper.MIDI_InsertCC(take,false,false,ppq,0xC0,ch,math.max(0,math.min(127,program or 0)),0); for _,n in ipairs(notes) do local s=reaper.MIDI_GetPPQPosFromProjTime(take,reaper.TimeMap2_QNToTime(0,n.start_qn)); local e=reaper.MIDI_GetPPQPosFromProjTime(take,reaper.TimeMap2_QNToTime(0,n.start_qn+n.duration_qn)); reaper.MIDI_InsertNote(take,false,false,s,e,n.channel,n.pitch,n.velocity,true) end; reaper.MIDI_Sort(take); return item end
local function apply_composition(text,items,tracks)
 local src,targets={},{ }; for _,it in ipairs(items) do src[it.guid]=it end; for _,t in ipairs(tracks or {}) do targets[t.guid]=t.track end; local jobs={}
 for line in text:gmatch("[^\r\n]+") do local k,g,rest=trim(line):match("^CS|([^|]+)|([^|]+)|?(.*)$"); if not k then return nil,"Unerwartete Kompositionsantwort." end; if k=="unchanged" then if not src[g] then return nil,"Unbekannte Quelle." end elseif k=="revised" or k=="target" or k=="track" or k=="new" then local name,pg,nt=rest:match("^([^|]+)|(%d+)|(.+)$"); local notes=parse_notes(nt); local program=tonumber(pg); if not name or not notes or not program or program<0 or program>127 then return nil,"Ungültige Kompositionsdaten." end; if (k=="revised" or k=="target") and not src[g] then return nil,"Unbekannte Quelle." end; if k=="track" and not targets[g] then return nil,"Unbekannte Zielspur." end; if k=="new" and g~="-" then return nil,"Ungültige neue Spur." end; jobs[#jobs+1]={kind=k,guid=g,name=trim(name),program=program_for_name(name,program),notes=notes} else return nil,"Unbekannter Ergebnistyp." end end
...
local function analysis_prompt(request,items,tracks) return [[Du analysierst das konkret übergebene MIDI-Material. Antworte musikalisch präzise als normaler Text. Erfinde nichts. Erzeuge kein MIDI und keine CS-Zeilen.]].."\nAUFTRAG:\n"..request.."\nMUSIK:\n"..music_context(items,tracks) end
local function composition_prompt(request,items,tracks) return [[Du komponierst Musik in Composition Studio. Nutze vorhandenes Material als musikalischen Kontext und erfülle den freien Auftrag eigenständig; füge keine unnötigen Regeln hinzu. Wenn weder MIDI-Material noch eine Zielspur vorhanden ist, handelt es sich bei einem entsprechenden Auftrag um eine freie Neukomposition: Erzeuge die benötigten neuen Spuren mit CS|new. TIME_SELECTION ist der markierte Zielbereich. Eine ausgewählte TRACK_GUID kann auch ohne ausgewähltes MIDI-Item direkt Ziel sein. Jedes neu erzeugte MIDI-Item MUSS einen passenden General-MIDI-Program-Change (0-127) erhalten. Verwende für benannte Instrumente den passenden GM-Wert, z.B. Violine=40, Cello=42, akustische Gitarre=24. Bei vorhandenen Zielspuren wird ein dort bereits vorhandener Program Change von Composition Studio bevorzugt übernommen.
...
PROGRAM ist die GM-Programmnummer 0-127. Nutze track bei einer ausdrücklich ausgewählten Zielspur, target bei der Spur eines ausgewählten Quell-Items, new für eine neue Stimme/Spur und revised für eine separate Variante. Bei einer freien Neukomposition mit mehreren Instrumenten erzeuge für jedes Instrument eine eigene CS|new-Zeile.]].."\nAUFTRAG:\n"..request.."\nMUSIK:\n"..music_context(items,tracks) end
local function process(request)
 local key=get_key(); if not key then add("KI","Kein OpenAI API-Key verfügbar."); return end; local items=selected_items(false); local tracks=selected_tracks(); local prompt=CONTROLLER.."\n\nBISHERIGER DIALOG:\n"..recent_dialog().."\n\nAKTUELLER AUFTRAG:\n"..request.."\n\nKOMPAKTER REAPER-KONTEXT:\n"..compact_context(items,tracks); local answer,err=run_openai(prompt,key); if not answer then add("KI",err); return end; answer=trim(answer)
 local chat=answer:match("^CHAT|(.*)$"); if chat then add("KI",trim(chat)); return end; local ask=answer:match("^ASK|(.*)$"); if ask then add("KI",trim(ask)); return end
 local awhy=answer:match("^NEED_ANALYSIS|(.*)$"); if awhy then local full=selected_items(true); if #full==0 and #tracks>0 then full=track_context_items(tracks) end; if #full==0 then add("KI","Für die Analyse ist kein MIDI-Material ausgewählt."); return end; local text,e=run_openai(analysis_prompt(request,full,tracks),key); if not text then add("KI",e); return end; add("KI",trim(text)); return end
 local why=answer:match("^NEED_MUSIC|(.*)$"); if why then local full=selected_items(true); if #full==0 and #tracks>0 then full=track_context_items(tracks) end; local comp,e=run_openai(composition_prompt(request,full,tracks),key); if not comp then add("KI",e); return end; local made,ae=apply_composition(comp,full,tracks); if not made then add("KI","Die musikalische Antwort konnte nicht sicher angewendet werden: "..tostring(ae)); return end; add("KI",string.format("Erledigt. %d neues bzw. überarbeitetes MIDI-Item wurde erzeugt.",#made)); return end
...
local function wrap_input_text(s,limit)
 limit=limit or 58; local out={}
 for line in (tostring(s or "").."\n"):gmatch("(.-)\n") do
  while #line>limit do local cut=limit; local part=line:sub(1,limit); local sp=part:match("^.*()%s+"); if sp and sp>math.floor(limit*0.55) then cut=sp end; out[#out+1]=line:sub(1,cut):gsub("%s+$",""); line=line:sub(cut+1):gsub("^%s+","") end
  out[#out+1]=line
 end
 return table.concat(out,"\n"):gsub("\n$","")
end
local function info_text() return "AKTUELLER STAND\n\nComposition Studio arbeitet direkt in REAPER.\n\nWAS IST NEU? – "..VERSION.."\n\n• Program Changes werden für benannte Instrumente zusätzlich in Composition Studio abgesichert; Violine, Cello, Gitarre usw. fallen nicht mehr still auf Piano zurück.\n• Vorhandene Zielspuren behalten weiterhin ihren vorhandenen Program Change.\n• Das Eingabefeld bricht lange Zeilen automatisch um, damit eingefügte Textpassagen vollständig sichtbar und bearbeitbar bleiben.\n• Die selektierbaren Chattexte aus test16 bleiben erhalten.\n\nTEST\n\nErzeuge z.B. Violine und Gitarre und prüfe die Program Changes. Kopiere anschließend einen längeren Absatz aus dem Chat ins Eingabefeld und prüfe den Zeilenumbruch." end
