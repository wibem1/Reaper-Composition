...
local function get_api_key()
  ...
  local ok,v=reaper.GetUserInputs(SCRIPT_NAME.." – Ersteinrichtung",1,"OpenAI API-Key:,extrawidth=320",""); if not ok then return nil end
  key=trim(v); if key=="" then return nil end; reaper.SetExtState(EXT_SECTION,EXT_KEY,key,true); return key
end
...
local function run_openai(prompt,key)
  local base=os.tmpname(); local req,res,code=base..".json",base..".out",base..".code"; local body='{"model":"gpt-5.6","input":"'..json_escape(prompt)..'"}'; if not write_file(req,body) then return nil,"Temporäre Anfrage konnte nicht geschrieben werden." end
  local cmd="/usr/bin/curl -sS --max-time 120 -o "..shell_quote(res).." -w '%{http_code}' -H "..shell_quote("Authorization: Bearer "..key).." -H 'Content-Type: application/json' --data-binary @"..shell_quote(req).." https://api.openai.com/v1/responses > "..shell_quote(code); os.execute(cmd); local status=trim(read_file(code)); local raw=read_file(res); os.remove(req); os.remove(res); os.remove(code); if status~="200" or not raw then return nil,"KI-Aufruf fehlgeschlagen (HTTP "..tostring(status)..")." end; local text=response_output_text(raw); if not text then return nil,"Keine Textantwort der KI gefunden." end; return text
end
local function parse_notes(text) local notes={}; for token in (text or ""):gmatch("[^;]+") do local a,b,c,d,e=token:match("^%s*([%d%.%-]+),([%d%.%-]+),(%d+),(%d+),(%d+)%s*$"); a,b,c,d,e=tonumber(a),tonumber(b),tonumber(c),tonumber(d),tonumber(e); if not(a and b and c and d and e) or a<0 or b<=0 or c<0 or c>127 or d<1 or d>127 or e<0 or e>15 then return nil end; notes[#notes+1]={start_qn=a,duration_qn=b,pitch=c,velocity=d,channel=e} end; return #notes>0 and notes or nil end
...
local function apply_results(results,items)
 local sources={}; for _,it in ipairs(items) do sources[it.guid]=it end; local created={}; reaper.Undo_BeginBlock2(0); reaper.PreventUIRefresh(1); local ok,err=xpcall(function() for _,r in ipairs(results) do if r.kind=="revised" then local src=sources[r.source]; local track_no=math.floor(reaper.GetMediaTrackInfo_Value(src.track,"IP_TRACKNUMBER")); local name=r.name.." [Variante]"; local tr=create_track(name,track_no); local item=create_midi_item(tr,name,r.notes); if not item then error("MIDI-Item konnte nicht erzeugt werden") end; created[#created+1]=item elseif r.kind=="new" then local tr=create_track(r.name); local item=create_midi_item(tr,r.name,r.notes); if not item then error("MIDI-Item konnte nicht erzeugt werden") end; created[#created+1]=item end end end,debug.traceback); reaper.PreventUIRefresh(-1); if not ok then reaper.Undo_EndBlock2(0,"Composition Studio – fehlgeschlagen",-1); reaper.Undo_DoUndo2(0); return nil,err end; if #created>0 then for i=0,reaper.CountMediaItems(0)-1 do reaper.SetMediaItemSelected(reaper.GetMediaItem(0,i),false) end; for _,it in ipairs(created) do reaper.SetMediaItemSelected(it,true) end end; reaper.UpdateArrange(); reaper.Undo_EndBlock2(0,"Composition Studio – KI-Komposition",-1); return created
end
local function add_message(role,text) history[#history+1]={role=role,text=text} end
local function process_request(request)
 local key=get_api_key(); if not key then add_message("KI","Kein OpenAI API-Key verfügbar."); return end; local items=selected_midi_items(); local answer,err=run_openai(build_prompt(request,context_text(items)),key); if not answer then add_message("KI",err); return end; local chat=answer:match("^%s*CHAT|(.*)$"); if chat then add_message("KI",trim(chat)); return end; local results,perr=parse_results(answer,items); if not results then add_message("KI","Die Antwort konnte nicht sicher angewendet werden: "..tostring(perr)); return end; local created,aerr=apply_results(results,items); if not created then add_message("KI","Das Ergebnis konnte nicht angewendet werden: "..tostring(aerr)); return end; if #created==0 then add_message("KI","Ich habe keine neue oder überarbeitete Stimme erzeugt. Das vorhandene Material blieb unverändert.") else add_message("KI",string.format("Erledigt. %d neues bzw. überarbeitetes MIDI-Item wurde in REAPER erzeugt. Das Original blieb erhalten.",#created)) end
end
