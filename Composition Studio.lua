...
local function version_parts(v) local a,b,t=v:match("^(%d+)%.(%d+)%-test(%d+)$"); return tonumber(a),tonumber(b),tonumber(t) end
local function version_is_newer(remote,localv) local ra,rb,rt=version_parts(remote); local la,lb,lt=version_parts(localv); if not (ra and la) then return false end; if ra~=la then return ra>la end; if rb~=lb then return rb>lb end; return rt>lt end
local function install_update()
 if busy then return end; busy=true; update_status="Update wird geladen …"; local tmp=os.tmpname()..".lua"; local code=os.tmpname()..".code"
 local sep=UPDATE_URL:find("?",1,true) and "&" or "?"; local url=UPDATE_URL..sep.."v="..enc and "" or ""
 url=UPDATE_URL.."?version_check="..tostring(os.time())
 local cmd="/usr/bin/curl -sS -L --max-time 60 -o "..shell_quote(tmp).." -w '%{http_code}' "..shell_quote(url).." > "..shell_quote(code)
 os.execute(cmd); local status=trim(read_file(code)); local fresh=read_file(tmp); os.remove(code); os.remove(tmp)
 ...
 if not remote_version or not fresh:find('local SCRIPT_NAME="Composition Studio"',1,true) then update_status="Update abgebrochen: heruntergeladene Datei ist ungültig."; busy=false; return end
 if remote_version==VERSION then update_status="Bereits aktuell: "..VERSION; busy=false; return end
 if not version_is_newer(remote_version,VERSION) then update_status="Kein neueres Update verfügbar. Lokal: "..VERSION..", GitHub: "..remote_version; busy=false; return end
 if SCRIPT_PATH=="" or not write_file(SCRIPT_PATH,fresh) then update_status="Update konnte nicht installiert werden."; busy=false; return end
 update_status="Update auf "..remote_version.." installiert. Neustart …"; restarting=true; open=false; reaper.SetExtState(EXT_SECTION,WINDOW_STATE_KEY,"1",true)
 reaper.defer(function() local ok,err=pcall(dofile,SCRIPT_PATH); if not ok then reaper.ShowConsoleMsg("Composition Studio Update-Neustart: "..tostring(err).."\n") end end)
end
...
local function get_key() local k=trim(reaper.GetExtState(EXT_SECTION,EXT_KEY)); if k~="" then return k end; local ok,v=reaper.GetUserInputs(SCRIPT_NAME.." – Ersteinrichtung",1,"OpenAI API-Key:,extrawidth=320",""); if not ok then return nil end; k=trim(v); if k~="" then reaper.SetExtState(EXT_SECTION,EXT_KEY,k,true); return k end end
local function run_openai(prompt,key) local b=os.tmpname(); local rq,rs,cd=b..".json",b..".out",b..".code"; if not write_file(rq,'{"model":"gpt-5.6","input":"'..json_escape(prompt)..'"}') then return nil,"Anfrage konnte nicht geschrieben werden." end; local cmd="/usr/bin/curl -sS --max-time 120 -o "..shell_quote(rs).." -w '%{http_code}' -H "..shell_quote("Authorization: Bearer "..key).." -H 'Content-Type: application/json' --data-binary @"..shell_quote(rq).." https://api.openai.com/v1/responses > "..shell_quote(cd); os.execute(cmd); local status=trim(read_file(cd)); local raw=read_file(rs); os.remove(rq); os.remove(rs); os.remove(cd); if status~="200" or not raw then return nil,"KI-Aufruf fehlgeschlagen (HTTP "..tostring(status)..")." end; return response_text(raw),nil end
...
local function compact_context(items) local l={string.format("Tempo %.2f BPM; selected MIDI items=%d",reaper.Master_GetTempo(),#items),time_selection_context()}; for i,it in ipairs(items) do l[#l+1]=string.format("ITEM %d id=%s track=%s take=%s rangeQN=%.3f..%.3f",i,it.guid,it.track_name,it.take_name,it.start_qn,it.end_qn) end; return table.concat(l,"\n") end
local function music_context(items) local l={compact_context(items)}; for _,it in ipairs(items) do for _,n in ipairs(it.notes) do l[#l+1]=string.format("N %s %.5f %.5f %d %d %d",it.guid,n.start_qn,n.duration_qn,n.pitch,n.velocity,n.channel) end end; return table.concat(l,"\n") end
...
local function apply_composition(text,items)
 local src={}; for _,it in ipairs(items) do src[it.guid]=it end; local jobs={}
 for line in text:gmatch("[^\r\n]+") do local k,g,rest=trim(line):match("^CS|([^|]+)|([^|]+)|?(.*)$"); if not k then return nil,"Unerwartete Kompositionsantwort." end
  if k=="unchanged" then if not src[g] then return nil,"Unbekannte Quelle." end
  elseif k=="revised" or k=="target" or k=="new" then local name,nt=rest:match("^([^|]+)|(.+)$"); local notes=parse_notes(nt); if not name or not notes or ((k=="revised" or k=="target") and not src[g]) or (k=="new" and g~="-") then return nil,"Ungültige Kompositionsdaten." end; jobs[#jobs+1]={kind=k,guid=g,name=trim(name),notes=notes}
  else return nil,"Unbekannter Ergebnistyp." end
 end
 reaper.Undo_BeginBlock2(0); local made={}; local ok,err=xpcall(function() for _,j in ipairs(jobs) do local tr; if j.kind=="revised" then local no=math.floor(reaper.GetMediaTrackInfo_Value(src[j.guid].track,"IP_TRACKNUMBER")); tr=create_track(j.name.." [Variante]",no) elseif j.kind=="target" then tr=src[j.guid].track else tr=create_track(j.name) end; local it=create_midi(tr,j.name,j.notes); if not it then error("MIDI konnte nicht erzeugt werden") end; made[#made+1]=it end end,debug.traceback)
 if not ok then reaper.Undo_EndBlock2(0,"Composition Studio – fehlgeschlagen",-1); reaper.Undo_DoUndo2(0); return nil,err end; reaper.UpdateArrange(); reaper.Undo_EndBlock2(0,"Composition Studio – KI-Komposition",-1); return made
end
...
Wenn der Auftrag ausdrücklich die vorhandene ausgewählte Stimme/Spur als Ziel bezeichnet, verwende CS|target mit der SOURCE_GUID des ausgewählten MIDI-Items. Das erzeugte Material wird dann auf derselben REAPER-Spur angelegt. Wenn ausdrücklich eine neue Stimme/Spur gewünscht ist, verwende CS|new. Für eine Variante auf einer separaten Spur verwende CS|revised. Entscheide nur aus dem freien Auftrag; bei fehlender Eindeutigkeit bevorzuge keine Änderung des bisherigen Verhaltens.
Antworte ausschließlich mit technischen Ergebniszeilen:
CS|unchanged|SOURCE_GUID
CS|target|SOURCE_GUID|NAME|startQN,durationQN,pitch,velocity,channel;...
...
Für target/revised muss SOURCE_GUID angeboten sein; für new '-'. NAME enthält kein |.
]].."AUFTRAG:\n"..request.."\nMUSIK:\n"..music_context(items)
end
local function process(request) local key=get_key(); if not key then add("KI","Kein OpenAI API-Key verfügbar."); return end; local items=selected_items(false); local prompt=CONTROLLER.."\n\nBISHERIGER DIALOG:\n"..recent_dialog().."\n\nAKTUELLER AUFTRAG:\n"..request.."\n\nKOMPAKTER REAPER-KONTEXT:\n"..compact_context(items); local answer,err=run_openai(prompt,key); if not answer then add("KI",err); return end; answer=trim(answer); local chat=answer:match("^CHAT|(.*)$"); if chat then add("KI",trim(chat)); return end; local ask=answer:match("^ASK|(.*)$"); if ask then add("KI",trim(ask)); return end; local why=answer:match("^NEED_MUSIC|(.*)$"); if why then local full=selected_items(true); local comp,cerr=run_openai(composition_prompt(request,full),key); if not comp then add("KI",cerr); return end; local made,aerr=apply_composition(comp,full); if not made then add("KI","Die musikalische Antwort konnte nicht sicher angewendet werden: "..tostring(aerr)); return end; add("KI",string.format("Erledigt. %d neues bzw. überarbeitetes MIDI-Item wurde erzeugt.",#made)); return end; if answer:match("^ACTION|") then local a,perr=parse_action(answer,items); if not a then add("KI","Ich führe nichts aus: "..perr); return end; local ok,aerr=execute_action(a); if not ok then add("KI","Die Aktion wurde nicht ausgeführt: "..tostring(aerr)); return end; add("KI",trim(a.desc).." – erledigt. REAPER Undo kann die Änderung rückgängig machen."); return end; add("KI","Ich konnte den Auftrag nicht eindeutig einem sicheren Vorgang zuordnen und habe nichts verändert.") end
...
local function info_text() return "AKTUELLER STAND\n\nComposition Studio arbeitet direkt in REAPER. GPT-5.6 nutzt Dialog, ausgewählte MIDI-Items und die REAPER-Zeitauswahl als Kontext.\n\nWAS IST NEU? – "..VERSION.."\n\n• Ein ausgewähltes MIDI-Item kann jetzt seine vorhandene REAPER-Spur als Ziel für neues komponiertes Material bestimmen.\n• Auftrag wie: 'Setze diese Stimme im markierten Bereich fort' legt das Ergebnis auf derselben Spur an.\n• 'Ergänze eine neue Stimme' behält das bisherige Verhalten und erzeugt eine neue Spur.\n• Varianten können weiterhin auf einer separaten Spur entstehen.\n• Der Update-Abruf erhält jetzt einen Zeitstempel, damit eine gerade veröffentlichte Version nicht mehr wegen Raw-GitHub-Caching als 'bereits aktuell' erscheint.\n\nTEST\n\nEin MIDI-Item auswählen, einen freien Zeitbereich markieren und schreiben: Setze diese Stimme im markierten Bereich fort. Das neue Item soll auf derselben Spur und im markierten Bereich entstehen." end
local function draw_history() if info_visible then reaper.ImGui_TextWrapped(ctx,info_text()); return end; for i=chat_start,#history do local m=history[i]; reaper.ImGui_TextWrapped(ctx,m.role..": "..m.text); reaper.ImGui_Spacing(ctx) end; if history_mode then reaper.ImGui_Separator(ctx); if reaper.ImGui_Button(ctx,"Verlauf löschen") then clear_saved_history() end end end
